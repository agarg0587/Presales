...
int num;
	...
	ns_save_string (userTable[bindTable[num].unum]uname, "Uname");
...
{
	strcpy( deviceTable[devnum].cookie1, ns_eval_string("{scookie}"));
	strcpy( deviceTable[devnum].cookie2, ns_eval_string("{dcookie}"));
}
/**********************************************************************
...
Code:
Udist[], calc Dsum and DWsum
USum UWsum Dsum DWsum ISum IWsum

Wsum
for (i = 0; i < 10; i++) {
	reduce all UDist[i] *=  UWsum/DWsum;
	Wsum += (i+1) * UDist[i]
} 

adjust_dist( int * dist, int DWsum) {
left = WDsum - Wsum
distribute over 2 or 1 
 Udist [1] = left/4 
 Udist [0] = left - (left/4) *2;
}

adjust_dist for U and I.
